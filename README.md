...
![SnapSheet Preview](public/screenshot.png)
...
### 交互增强（2026-06-22 更新）
- 🖱️ **表头交互**：点击行号/列标可选中整行/整列；行/列表头右键支持插入、删除、隐藏/显示、自动调整尺寸
- 🙈 **隐藏行列**：支持隐藏/显示行列，自动筛选会利用该能力折叠不满足条件的行
- 🤖 **公式自动补全**：输入 `=` 后弹出函数列表，支持方向键选择、Enter/Tab 插入，附带中文说明
- 🔍 **XLOOKUP**：新增 `XLOOKUP` 查找函数，支持精确匹配、近似匹配及自定义未找到返回值
- 🗂️ **自动筛选**：选中表头和数据区域后，可开启自动筛选，通过表头下拉菜单勾选显示值
- 📈 **图表插入**：支持柱状图、折线图、面积图、散点图、饼图，可拖拽移动、缩放、双击编辑
- 🌐 **在线体验**：[snapsheet.asia](https://snapsheet.asia)

### 桌面客户端
- 🖥️ **Electron 桌面版**：支持 macOS（Apple Silicon / Intel），可离线运行
- 🌐 **官方网站**：根目录 `index.html` 为产品官网，构建后输出为 `dist/index.html`，支持在线试用与下载
...
│   ├── main.tsx         # 应用入口
│   └── index.css        # 全局样式（主题变量）
├── index.html           # 产品官网落地页（构建后输出 dist/index.html）
├── app/
│   └── index.html       # Web 应用入口（构建后输出 dist/app/index.html）
├── .github/
│   └── workflows/
...
- `SnapSheet-1.0.0-arm64.dmg` — Apple Silicon（M1/M2/M3/M4）
- `SnapSheet-1.0.0.dmg` — Intel 处理器 Mac

### 官网

```bash
# 官网为纯静态页面，构建后输出为 dist/index.html
...
### v1.0.0 (2026-06-22)
- 🖱️ 表头交互：点击行号/列标选中整行/整列，右键支持插入、删除、隐藏/显示、自动调整尺寸
- 🙈 支持隐藏/显示行列，键盘方向键自动跳过隐藏行列
- 🤖 公式自动补全：输入 `=` 后弹出函数列表与中英文说明
- 🔍 新增 `XLOOKUP` 查找函数
- 🗂️ 新增自动筛选：表头下拉勾选显示值，不满足条件的行自动折叠
- 📈 新增图表插入：柱状图、折线图、面积图、散点图、饼图，支持拖拽移动、缩放与双击编辑
- 🌐 官网首页更新，补充新功能介绍与 v1.0.0 下载链接
- ⬆️ 版本号升级至 1.0.0
...
- 🏗️ 新增 SnapSheet 产品官网（根目录 `index.html`），支持在线试用与 Mac 下载
...
### Q: 如何更新官网中的下载链接？
A: 修改根目录 `index.html` 中下载按钮的 `href`，指向线上 DMG 文件地址即可。
