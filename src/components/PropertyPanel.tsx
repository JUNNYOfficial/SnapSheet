...
export default function PropertyPanel({ isOpen, onClose }: PropertyPanelProps) {
  ...
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div className="mb-4">
      <div className="text-[10px] font-medium uppercase tracking-wider mb-2" style={{ color: 'var(--ss-header-text)' }}>
        {title}
      </div>
  ...
  const PanelButton = ({ onClick, icon, label, disabled }: { onClick: () => void; icon: React.ReactNode; label: string; disabled?: boolean }) => (
    <button
      onClick={onClick}
      disabled={disabled}
      className="w-full flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-colors"
      style={{
        color: disabled ? 'var(--ss-header-text)' : 'var(--ss-toolbar-text)',
        background: disabled ? 'transparent' : 'var(--ss-input-bg)',
        fontFamily: 'SimSun, 宋体, SimHei, 黑体, sans-serif',
      }}
      onMouseEnter={(e) => { if (!disabled) (e.currentTarget as HTMLButtonElement).style.background = 'var(--ss-toolbar-hover)'; }}
      onMouseLeave={(e) => { if (!disabled) (e.currentTarget as HTMLButtonElement).style.background = 'var(--ss-input-bg)'; }}
    >
      {icon}
      {label}
    </button>
  ...
  return (
    <div className="fixed top-0 right-0 bottom-0 w-72 border-l z-50 overflow-hidden flex flex-col" style={{ borderColor: 'var(--ss-toolbar-border)', background: 'var(--ss-toolbar-bg)' }}>
      <div className="flex items-center justify-between px-3 py-2 border-b" style={{ borderColor: 'var(--ss-toolbar-border)' }}>
        <span className="text-sm font-medium" style={{ color: 'var(--ss-cell-text)', fontFamily: 'SimSun, 宋体, SimHei, 黑体, sans-serif' }}>
          属性面板
        </span>
        <button
          onClick={onClose}
          className="flex h-6 w-6 items-center justify-center rounded hover:bg-[var(--ss-toolbar-hover)] transition-colors"
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3">
        {activeTab === 'format' && (
          <>
            ...
            <Section title="背景色">
              <PanelButton onClick={() => store.getState().applyStyleToSelection({ bgColor: '#f5f5f5' })} icon={<span className="w-3 h-3 rounded" style={{ background: '#f5f5f5', border: '1px solid var(--ss-input-border)' }} />} label="浅灰" />
              <PanelButton onClick={() => store.getState().applyStyleToSelection({ bgColor: '#e5e5e5' })} icon={<span className="w-3 h-3 rounded" style={{ background: '#e5e5e5', border: '1px solid var(--ss-input-border)' }} />} label="中灰" />
              <PanelButton onClick={() => store.getState().applyStyleToSelection({ bgColor: '#262626', color: '#ffffff' })} icon={<span className="w-3 h-3 rounded" style={{ background: '#262626', border: '1px solid var(--ss-input-border)' }} />} label="黑底白字" />
