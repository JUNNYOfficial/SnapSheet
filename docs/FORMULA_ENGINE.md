...
SnapSheet 的公式引擎是一个基于词法分析、语法解析和表达式求值的完整计算系统。它支持 50+ 种内置函数，自动依赖追踪，以及缓存优化。
...
│  [EQUALS, SUM,        (FUNCTION              数值/字符串/布尔   │
│   PAREN_OPEN,         NAME:SUM,                               │
│   RANGE,              ARGUMENTS:                             │
│   PAREN_CLOSE]         (RANGE A1:B10))                         │
│                                 ↓                              │
│                       依赖追踪 & 缓存                           │
│                       记录依赖单元格                            │
│                       更新时智能重算                            │
...
在 `src/engine/Evaluator.ts` 中的 `functions` Map 中添加：
...
在 `src/engine/Evaluator.ts` 中添加运算符处理：
